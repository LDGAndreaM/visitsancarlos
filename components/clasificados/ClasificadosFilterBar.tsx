...
import { CONDITION_OPTIONS, PRICE_OPTIONS, SORT_KEYS, SORT_LABELS, type ClasificadoCategory, type SortKey } from "@/lib/clasificadosData";

export type ViewMode = "grid" | "list" | "map";

type ClasificadosFilterBarProps = {
  filterPrice: string;
...
export default function ClasificadosFilterBar({
  filterPrice,
  onFilterPriceChange,
  filterCondition,
    ...
    <section style={{ padding: "20px 48px 0" }}>
      <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <select value={filterCondition} onChange={(e) => onFilterConditionChange(e.target.value)} style={selectStyle}>
            {CONDITION_OPTIONS.map((c) => (
              <option key={c} value={c}>
                {c === "Todas" ? "Condición: todas" : c}
              </option>
            ))}
          </select>
          {filterCategory && (
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 8, border: "1px solid #DCEEEF", borderRadius: 999, background: "#F4FAFB", padding: "9px 16px", fontSize: 13, fontWeight: 600, color: "#009BA4", cursor: "pointer" }}
            >
              {filterCategory} ✕
            </button>
          )}
